#!/usr/bin/env python3
"""
StemPlay Library Premium
Interface limpa, sem emojis, compativel com Windows
"""
import re, json, sys
...
def main():
    ...
    if not Path(input_file).exists():
        print(f"  [ERRO] {input_file} nao encontrado! Rode s3_god_mode.py primeiro.")
        return
...
.theme-btn{{width:42px;height:42px;border-radius:50%;background:var(--bg3);border:1px solid var(--border);color:var(--txt);cursor:pointer;font-size:.9rem;font-weight:700;display:flex;align-items:center;justify-content:center;transition:transform .2s}}
...
.card{{background:var(--card);backdrop-filter:blur(10px);border-radius:var(--radius);padding:1rem;color:var(--txt);border:1.5px solid var(--border);display:flex;flex-direction:column;gap:.6rem;transition:transform .15s,border-color .2s,box-shadow .2s;position:relative;overflow:hidden}}
...
.nicon{{font-size:1.1rem;font-weight:800}}
...
.empty-icon{{font-size:2rem;font-weight:800;margin-bottom:1rem;color:var(--accent2)}}
...
<div class="header">
  <div class="header-top">
    ...
    <button class="theme-btn" id="themeBtn" title="Alternar tema">Tema</button>
  </div>
  <div class="stats-row">
  ...
  D.forEach(c=>{{
    ...
    Object.entries(gs).forEach(([gn,items])=>{{
      ...
      items.forEach(i=>{{
        ...
        a.innerHTML=`<div class="card-top"><div class="card-info"><div class="card-name">${{i.display_name}}</div><div class="card-badges">${{badges}}</div></div></div><div class="card-actions"><a href="${{onlineUrl}}" target="_blank" rel="noopener" class="abtn btn-online">Online</a><button class="abtn btn-dl" data-url="${{i.url}}" data-name="${{i.display_name}}">Download</button><button class="abtn btn-fav${{isF?' is-fav':''}}" data-url="${{i.url}}">${{isF?'&#9733;':'&#9734;'}}</button></div>`;
...
function toggleFav(u){{const i=favs.indexOf(u);if(i>=0){{favs.splice(i,1);toast('Removido dos favoritos')}}else{{favs.push(u);toast('Adicionado aos favoritos')}}localStorage.setItem('sp_favs',JSON.stringify(favs));render()}}
...
render();
console.log('StemPlay Library carregada:',D.reduce((s,c)=>s+c.items.length,0),'materiais em',D.length,'cursos');
</script>
</body>
</html>"""
    ...
    print("  [INFO] Features: Download | Online | Favoritos | Tema | Busca | Filtros | Mobile")
