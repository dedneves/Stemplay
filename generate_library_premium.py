...
def parse_pdf_info(url):
    ...
    return {
        ...
        "checkpoint_num": checkpoint_num, "sort_key": sort_key, "group_name": group_name
    }
...
function renderPlaceholder(course,type){{const letter=course.charAt(0).toUpperCase();const bg=courseGradient(course);return`<div class="thumb-wrap"><div class="placeholder" style="background:${{bg}}"><div class="ph-letter">${{letter}}</div><div class="ph-type">${{type}}</div><div class="ph-course">${{course}}</div></div></div>`}}
function render(){{const app=$('app');app.innerHTML='';let vc=0;const sl=cS.toLowerCase();D.forEach(c=>{{const fi=c.items.filter(i=>{{const ms=!sl||i.course.toLowerCase().includes(sl)||i.display_name.toLowerCase().includes(sl)||i.group_name.toLowerCase().includes(sl);const mf=cF==='all'||i.material_type===cF;if(cV==='fav')return favs.has(i.url);return ms&&mf}});if(!fi.length)return;vc+=fi.length;const gs={{}};fi.forEach(i=>{{(gs[i.group_name]=gs[i.group_name]||[]).push(i)}});const ce=document.createElement('div');ce.className='course';ce.innerHTML=`<div class="chdr"><div class="ctitle">${{c.name}}</div><div class="ccount">${{fi.length}}</div><div class="carrow">&#9660;</div></div><div class="cbody"></div>`;const cb=ce.querySelector('.cbody');Object.entries(gs).forEach(([gn,items])=>{{const gt=document.createElement('div');gt.className='gtitle';gt.textContent=gn+' \\u00b7 '+items.length+' itens';cb.appendChild(gt);const gr=document.createElement('div');gr.className='grid';items.forEach(i=>{{const a=document.createElement('div');a.className='card '+i.material_type.toLowerCase();const bc='b-'+i.material_type.toLowerCase();const isF=favs.has(i.url);const onlineUrl='https://reader.stemplay.io/?file='+encodeURIComponent(i.url)+'&userId=k3s';let badges=`<span class="badge ${{bc}}">${{i.material_type}}</span>`;if(i.module_num)badges+=`<span class="badge" style="background:#8b5cf6;color:#fff">M${{i.module_num}}</span>`;if(i.unit_num)badges+=`<span class="badge" style="background:#ec4899;color:#fff">U${{i.unit_num}}</span>`;a.innerHTML=`${{renderPlaceholder(i.course,i.material_type)}}<div class="card-body"><div class="card-name">${{i.display_name}}</div><div class="card-badges">${{badges}}</div><div class="card-actions"><a href="${{onlineUrl}}" target="_blank" rel="noopener" class="abtn btn-online">Online</a><button class="abtn btn-dl">Download</button><button class="abtn btn-fav${{isF?' is-fav':''}}">${{isF?'\\u2605':'\\u2606'}}</button></div></div>`;a.querySelector('.btn-dl').onclick=e=>{{e.stopPropagation();downloadPDF(i.url,i.display_name)}};a.querySelector('.btn-fav').onclick=e=>{{e.stopPropagation();toggleFav(i.url)}};gr.appendChild(a)}});cb.appendChild(gr)}});ce.querySelector('.chdr').onclick=()=>ce.classList.toggle('open');app.appendChild(ce)}});$('sT').textContent=D.reduce((s,c)=>s+c.items.length,0);$('sC').textContent=D.length;$('sV').textContent=vc;if(!vc){{app.innerHTML=`<div class="empty"><div class="empty-icon">${{cV==='fav'?'FAV':'BUSCA'}}</div><h3>${{cV==='fav'?'Nenhum favorito':'Nada encontrado'}}</h3><p style="margin-top:.5rem;font-size:.85rem;color:var(--txt2)">${{cV==='fav'?'Toque na estrela para favoritar':'Tente outra busca'}}</p></div>`}}}}
...
let searchTimer;$('searchBox').oninput=e=>{{clearTimeout(searchTimer);searchTimer=setTimeout(()=>{{cS=e.target.value;render()}},200)}};
...
const savedTheme=localStorage.getItem('sp_theme')||'dark';document.documentElement.setAttribute('data-theme',savedTheme);$('themeBtn').innerHTML=savedTheme==='dark'?'\\u263E':'\\u2600';render();
console.log('StemPlay Library v11 |',D.reduce((s,c)=>s+c.items.length,0),'materiais em',D.length,'cursos');
</script>
</body>
</html>"""
